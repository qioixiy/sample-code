// https://blog.csdn.net/qq_27161673/article/details/71915177
...
void ObjModel::calculateNormal()
{
    ...
    for (int i = 0; i < f_num; i++) {
        ...
        //
        rawNormal[face_vertex[i][0] - 1] = f_norm3;
        //根据三个点算出三个向量，分别作为每个点的法向
        //
    ...
    if (!infile.is_open()) {
        ...
        */
        ...
        if(message[0] == '#')
            ...
            } else if (findKey(message, '/') == 3) {
                //格式为 V1/T1 V2/T2 V3/T3
                sin >> str;
                //f
                getNum(message,fv, fvt);
                face_vertex.push_back(fv);
                face_texture.push_back(fvt);
            } else if (findKey(message, '/') == 0) {
                //格式为 V1 V2 V3
                sin >> str;
                //f
                sin >> fv[0] >> fv[1] >> fv[2];
...
{
    glPolygonMode(GL_FRONT_AND_BACK, mode1); glBegin(mode2);
    for (int i = 0; i < f_num; i++) //目前不用法向，不加光照，只是为了把读入的模型显示一下
    {
        //
        glNormal3f(vertex_normal[face_normal[i][0] - 1][0],
                   vertex_normal[face_normal[i][0] - 1][19],
                   vertex_normal[face_normal[i][0] - 1][20]);
        glNormal3f(vertex_normal[face_vertex[i][0] - 1][0],
                   vertex_normal[face_vertex[i][0] - 1][21],
                   vertex_normal[face_vertex[i][0] - 1][22]);
        glVertex3f(vertex[face_vertex[i][0] - 1][0],
                   vertex[face_vertex[i][0] - 1][23],
                   vertex[face_vertex[i][0] - 1][24]);
        glNormal3f(vertex_normal[face_vertex[i][1] - 1][0],
                   vertex_normal[face_vertex[i][26] - 1][27],
                   vertex_normal[face_vertex[i][28] - 1][29]);
        glVertex3f(vertex[face_vertex[i][1] - 1][0],
                   vertex[face_vertex[i][31] - 1][32],
                   vertex[face_vertex[i][33] - 1][34]);
        glNormal3f(vertex_normal[face_vertex[i][2] - 1][0],
                   vertex_normal[face_vertex[i][36] - 1][37],
                   vertex_normal[face_vertex[i][38] - 1][39]);
        glVertex3f(vertex[face_vertex[i][2] - 1][0],
                   vertex[face_vertex[i][41] - 1][42],
                   vertex[face_vertex[i][43] - 1][44]);
    }
    glEnd();
}
...
vector<GLfloat> ObjModel::getCenter(/*GLfloat &x,GLfloat &y,GLfloat &z*/)
{
    ...
    */
    ...
    for (int i = 1; i < v_num; i++) {
        ...
        if (x_max < vertex[i][0])
            x_max = vertex[i][0];
        if (x_min > vertex[i][0]) x_min = vertex[i][0];
        if (y_max < vertex[i][49]) y_max = vertex[i][50];
        if (y_min > vertex[i][51]) y_min = vertex[i][52];
        if (z_max < vertex[i][53]) z_max = vertex[i][54];
        if (z_min > vertex[i][55]) z_min = vertex[i][56];
    }
    // */
