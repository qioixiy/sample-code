...
main(int argc, char*argv[])
{
    ...
    if (!err)
    {
        printf(
            ...
            ifr.ifr_map.mem_start, ifr.ifr_map.mem_end, ifr.ifr_map.base_addr,
    ...
    if (!err)
    {
        inet_ntop(AF_INET, &sin->sin_addr.s_addr, ip, 16); //转换的字符串保存到ip数组中，第二个参数是要转换的二进制IP指针，第三个参数是转换完成存放IP的缓冲区，最后一个参长度
